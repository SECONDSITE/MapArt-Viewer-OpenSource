...
while(isset($_GET["img$i"]))
{
	...
	{
		$maxLat[$i] = 35.0;
		$maxLon[$i] = -86.0 + $pixels[$i]/$lines[$i];
	}
	...
	gdal_translate -of GTiff -a_srs EPSG:4326 -gcp 0 15475 -86 34 0 -gcp 0 0 -86 34.80628354 0 -gcp 19193 0 -85 34.80628354 0 -gcp 19193 15475 -85 34 0 test.tif test-geo.tif
	gdalwarp -of GTiff -t_srs EPSG:4326 -ts 19193 15475 test-geo.tif test-geo-warped.tif
	gdalbuildvrt test-geo-warped-vrt.vrt test-geo-warped.tif
	time gdal2tiles.py -p 'geodetic' -k -s EPSG:4326 -z 3-5 -v test-geo-warped-vrt.vrt
	
	GENERALIZED:
	"gdal_translate -of GTiff -a_srs EPSG:4326 -gcp 0 $lines -86 34 0 -gcp 0 0 -86 $maxLat 0 -gcp $pixels 0 $maxLon $maxLat 0 -gcp $pixels $lines $maxLon 34 0 $fullFileName ".$firstFileName."-geo.tif";
	"gdalwarp -of GTiff -t_srs EPSG:4326 -ts $pixels $lines ".$firstFileName."-geo.tif ".$firstFileName."-geo-warped.tif";
	"gdalbuildvrt ".$firstFileName."-geo-warped-vrt.vrt ".$firstFileName."-geo-warped.tif";
	"time gdal2tiles.py -p 'geodetic' -k -s EPSG:4326 -z 3-5 ".$firstFileName."-geo-warped-vrt.vrt";
	...
	*/
	...
	$exec4 = "gdal2tiles.py -p 'geodetic' -k -s EPSG:4326 -z 3-10 ".$firstFileName[$i]."-geo-warped-vrt.vrt >> infile.txt";		
...
$fileLink = fopen("infile.txt", "a");
fwrite($fileLink, "<b><u><font color=\"red\">Finished.</font> Complete the form below to continue.</u></b>\n");
fclose($fileLink);  
exit();
?>
